...
'''
    ...
    install         Default target and copies it to build/glpd (default)
...
GCC 5: If the gcc toolchain is used, gcc version 5 or better is required. On
    linux distros that ship with gcc 4 (ubuntu < 15.10), glpd will force gcc
    to use gcc4's ABI (there was an ABI change between versions). This allows us
    to use the package manager to install glpd dependencies. It also means if
...
Clang on linux: Clang cannot use the new gcc 5 ABI (clang does not know about
    ...
    link glpd in this scenario (use the `--static` with scons), as dynamic
...
def check_openssl():
    ...
    if parse_time(d) < parse_time(build_time):
        raise Exception('Your openSSL was built on %s; '
                        'glpd needs a version built on or after %s.'
                        % (line, build_time))
...
for tu_style in ['classic', 'unity']:
    for toolchain in all_toolchains:
        for variant in variants:
            ...
            target = env.Program(
                target=os.path.join(variant_dir, 'glpd'),
...
vcxproj = base.VSProject(
    os.path.join('Builds', 'VisualStudio2015', 'GlpD'),
    source = [],
...
# Build the list of glpd source files that hold unit tests
